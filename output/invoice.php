    ...
    <div class="flex flex-col ">
        ...
        <ul class="flex-1 overflow-y-auto">
            ...
            <li class="flex justify-between items-center py-2 px-4 border-b border-gray-500">
                ...
                <button onclick="updateCart('<?php echo $row['foodID'];?>', 'remove')" class="bg-pink-700 hover:bg-pink-50 hover:text-black text-white font-raleway max-w-fit mr-1 py-1 px-2 rounded-md focus:outline-black focus:ring-2 focus:ring-pink-400 w-8 hover:translate-0 hover:transition-shadow mt-4">-</button>

                <span id="quantity-<?php echo $row['foodID'];?>" class="text-pink-700 font-bold text-center w-6"><?php echo $row["quantity"];?> </span>

                <button onclick="updateCart('<?php echo $row['foodID'];?>', 'add')" class="bg-pink-700 hover:bg-pink-50 hover:text-black text-white font-raleway max-w-fit ml-1 py-1 px-2 rounded-md focus:outline-black focus:ring-2 focus:ring-pink-400 w-8 hover:translate-0 hover:transition-shadow mt-4">+</button>
                </div>
            </li>
            <?PHP 
            }}
...
<div class="flex flex-col md:flex-row justify-center items-center mx-auto m-5">
    <a href="../output/homepage.php" class="bg-gray-100 hover:bg-pink-700 hover:text-white text-black font-raleway py-3 px-5 m-4 rounded-full border-pink-600 focus:outline-black focus:ring-2 focus:ring-pink-400 max-w-fit hover:translate-0 hover:transition-shadow md:mx-2">
        Back to menu
    </a>
    <a href="checkout.php?price=<?php echo $total?>" class="bg-pink-700 hover:bg-gray-100 hover:text-black text-white font-raleway py-3 px-5 m-4 rounded-full focus:outline-4 focus:outline-pink-700 focus:ring-2 focus:ring-pink-400 max-w-sm hover:translate-0 hover:transition-shadow md:mx-2">
        Proceed to Payment
    </a>
</div>
